...
public class EnemyAgent : MonoBehaviour
{
    ...
    // Prevents enemy from getting health if they are attacked
    //  unless it's been healthGetDelay seconds
    float healthGetStamp = -100f;
    float healthGetDelay = 3;
        ...
        else
        {
            ...
            else if (wasAttacked)
            {
                ...
                if (target != null)
                {
                    ...
                    GetComponent<NavMeshAgent>().stoppingDistance = 10;
                ...
                // If we're allowed to go after health
                if(health < 30 && Time.time > healthGetStamp + healthGetDelay)
                {
                    // Find all healthpacks
                    healthPacks = GameObject.FindGameObjectsWithTag("HealthPickup");
                    float closestDistance = 100000f;
                    GameObject closestPack = null;

                    // If there are health packs
                    if(healthPacks.Length != 0)
                    {
                        ...
                        {
                            if(Vector3.Distance(transform.position, pack.transform.position) < closestDistance)
                            {
                                closestDistance = Vector3.Distance(transform.position, pack.transform.position);
                                closestPack = pack;
                            }
                        }

                        // Set our destination to this pack
                        currentDestination = closestPack.transform.position;
    ...
    {
        //Gizmos.DrawCube(gameObject.GetComponent<NavMeshAgent>().destination, new Vector3(10, 10, 10));
    }
}
